# create cdf plots for gene expression and decay rates comparisons for miR-29 targets. Fig 3H-I.
...
cleanCd8_counts_wholegene_cpm_noLow = cleanCd8_counts_wholegene_cpm[rowSums(cleanCd8_counts_wholegene_cpm < 1) == 0 , , drop = FALSE] # 12874 genes
...
plot_barplot_lineplots_cleanCd8project <- function(matrix, seed, network_path, plot_title, file_name){
  ...
  dir.create(file.path(paste0('make_figures/ct02_ms2_4ormore/benchmark/let7/', 
                              "/cleanCd8project_validatetargets_pct_TsOnly_matchedscore/", miRNA_name, "/")), recursive = TRUE, showWarnings = FALSE)
  ...
  print(table(miRNA_bothnopct == miRNA_inf_noWeak))
  print('if all TRUE, miRNA_bothnopct (targets from Inf+Ts_nopct is the same with targets from Inf')

  # get cwcs distribution of inf targets
  ...
  bg_noTargets = setdiff(bg_noTargets, miRNA_ts_matched_exp)
  ...
  miRNA_tsMatchedExp_dr_toMelt = log10(miRNA_tsMatchedExp_dr)
  ...
  miRNA_tsMatchedExp_dr_fc$fc = log2(rowMeans(miRNA_tsMatchedExp_dr_fc[,c("Clean_VM_rep1", "Clean_VM_rep2", "Clean_VM_rep3")])/rowMeans(miRNA_tsMatchedExp_dr_fc[,c("Clean_TN_rep1", "Clean_TN_rep2", "Clean_TN_rep3")]))
  ...
  ggplot(forplot, aes(fc, colour = type)) +
    ...
    values = c("black", "navyblue", "#52b2bf", "lightblue2", 'yellow4', 'yellow1'))+ 
    ggtitle(paste0(miRNA_name, " Targets\n", plot_title, " Fold Change"))
  ggsave(
    paste0('make_figures/ct02_ms2_4ormore/benchmark/let7/', 
           "/cleanCd8project_validatetargets_pct_TsOnly_matchedscore/", miRNA_name, "/", file_name, "_", file_path_sans_ext(basename(network_path)), "_lineplots_", miRNA_name, "_cleanCd8project_ncdacd.pdf"),
    plot = last_plot(),
    device = "pdf",
    width = 7,
    height = 3.5,
  ...
  png(file=paste0('make_figures/ct02_ms2_4ormore/benchmark/let7/', 
                  "/cleanCd8project_validatetargets_pct_TsOnly_matchedscore/", miRNA_name, "/", file_name, "_", file_path_sans_ext(basename(network_path)), "_lineplots_", miRNA_name, "_cleanCd8project_ncdacd.png"), width=1000, height=1000, res=120)
