        ...
        for meanEdgesPerGene = [2] % reaches the largest network
            for seed = [7,18,99,26,57] % runs five times with five different seeds
                for lambdaBias = [0.05] % strong reinforcement
                    ...
                    diaryFile = ['outputs/241023/mirnaSeed_tsonly_noSP8_wt_ct02_seed' num2str(seed) '/' name_this_run '.log'];
                    matlabDir = 'infTRN_lassoStARS-master_clChangedOutside';
                    outputFolder = ['outputs/241023/mirnaSeed_tsonly_noSP8_wt_ct02_seed' num2str(seed) '/' name_this_run ];
