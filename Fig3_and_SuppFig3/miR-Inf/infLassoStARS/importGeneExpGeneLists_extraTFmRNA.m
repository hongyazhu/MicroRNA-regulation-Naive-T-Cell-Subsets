function importGeneExpGeneLists_extraTFmRNA(normGeneExprFile,targGeneFile,potRegFile,NEWpotRegmRNAlevels,...
    tfaGeneFile,outMat)
% !!! modified from importGeneExpGeneLists. added NEWpotRegmRNAlevels as 
...
fclose(fid);
potRegs = C{1};
% read NEWpotRegmRNAlevels (TF mRNA levels from RNAseq)
fid = fopen(NEWpotRegmRNAlevels);
tline=fgetl(fid); % line 1
tline2 = fgets(fid); % line 2
totSamps = length(cellstr(strvcat(strsplit(tline2,'\t')))') - 1; % number of samples
conditionsc = cellstr(strvcat(strsplit(tline,'\t')))';
...
NEWgenesc = C{1};
NEWncounts = [C{2:end}];
[vars obs] = size(NEWncounts);

% get mRNA levels of potential regulators matrix
[xx, indsMat] = intersect((NEWgenesc),(potRegs));
potRegs_mRNA = cellstr(strvcat(NEWgenesc{indsMat}));
potRegMat_mRNA = NEWncounts(indsMat,:);
